...
def test_loader_node():
    ...
    # 2. Run Node
    result = load_memory_node(initial_state, MOCK_CONFIG)
    
    # 3. Verify
    print(f"✅ Output Keys: {result.keys()}")
    print(f"✅ Loaded Context: {result.get('user_context')}")
    assert "user_context" in result, "Loader failed to return user_context"
